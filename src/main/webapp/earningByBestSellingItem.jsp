...
<p> Best selling items are based on how many times that item was sold in an auction</p>
	...
	try {
		...
		String str = "SELECT p.product_id, p.category, p.brand, p.color, p.year, count(*) FROM auction a, product p WHERE a.status='close' and a.current_bid>=a.min_price and a.product_id=p.product_id group by p.product_id order by count(*) desc limit 5";
